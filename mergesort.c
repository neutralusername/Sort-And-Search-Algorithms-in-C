...
void mergeSortArray(int *input, int size, int asc) {
    if(size > 1) {//if the array is empty or has only one element.  This is the base case for the recursion. If the array is empty or has only one element, it is already sorted.
        ...
        mergeSortArray(left, size / 2, asc); //sort the left array. This is the recursive call. The left array will be sorted when the function returns.
        mergeSortArray(right, size - size / 2, asc); //sort the right array. This is the recursive call. The right array will be sorted when the function returns. 
        ...
        else for(int i = 0; i < size; i++) { //for each element in the input array
            if(leftIndex >= size / 2) { //if the left array is finished
                input[i] = right[rightIndex]; //copy the element from the right array
                rightIndex++; //increment the right index
            } else if(rightIndex >= size - size / 2) { //if the right array is finished
